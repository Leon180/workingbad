...
func NewServer(svc *repository.Service, cfg config.Web) (*Server, error) {
	...
	s.mux.HandleFunc("GET /nodes/new", s.handleNodeNewForm)
	s.mux.HandleFunc("POST /nodes", s.handleNodeCreate)
	s.mux.HandleFunc("GET /nodes/{id}", s.handleNodeDetail)
	s.mux.HandleFunc("GET /nodes/{id}/edit", s.handleNodeEditForm)
	s.mux.HandleFunc("POST /nodes/{id}", s.handleNodeUpdate)
	...
	s.mux.HandleFunc("GET /entries/{id}/labels", s.handleGetLabels)
	s.mux.HandleFunc("POST /entries/{id}/labels", s.handleSetLabels)
